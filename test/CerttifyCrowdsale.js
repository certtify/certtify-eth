...
contract('CerttifyCrowdsale', function(accounts) {
    ...
    it('Crowdsale contract deployed successfully with all variables set as expected', function(done) {
        ...
        Crowdsale.new(_timestampStage1, _timestampStage2, _timestampStage3, _timestampEndTime, _weiCostOfTokenStage1, _weiCostOfTokenStage2, _weiCostOfTokenStage3, _wallet, _owner, _bountyAdmin, _founderTokenUnlockPhase1, _founderTokenUnlockPhase2, _founderTokenUnlockPhase3, _founderTokenUnlockPhase4, {
            ...
            assert.equal(capStage2, maxSupply.mul(0.47), "Stage-2 cap is not set correctly");
            var capStage3 = contractVars[14].valueOf();
            assert.equal(capStage3, maxSupply.mul(0.72), "Stage-3 cap is not set correctly");
    ...
    it('Cannot pre-sale after stage-1 is launched', function(done) {
        ...
        Crowdsale.new(getTimestamp(0), _timestampStage2, _timestampStage3, _timestampEndTime, _weiCostOfTokenStage1, _weiCostOfTokenStage2, _weiCostOfTokenStage3, _wallet, _owner, _bountyAdmin, _founderTokenUnlockPhase1, _founderTokenUnlockPhase2, _founderTokenUnlockPhase3, _founderTokenUnlockPhase4, {
            from: accounts[1]
        }).then(function(_instance) {
            return assertRevert(_instance.buyTokensPreSale(accounts[1], 1, {
                from: accounts[0]
    ...
    it('Handle a valid public buy token request in stage 2 ICO when the change is caused by timestamp', function(done) {
        ...
        Crowdsale.new(getTimestamp(0), getTimestamp(0), _timestampStage3, _timestampEndTime, _weiCostOfTokenStage1, _weiCostOfTokenStage2, _weiCostOfTokenStage3, _wallet, _owner, _bountyAdmin, _founderTokenUnlockPhase1, _founderTokenUnlockPhase2, _founderTokenUnlockPhase3, _founderTokenUnlockPhase4, {
            from: accounts[1]
        }).then(function(instance) {
    ...
    it('Handle a valid public buy token request in stage 3 ICO when the change is caused by timestamp', function(done) {
        ...
        Crowdsale.new(getTimestamp(0), getTimestamp(0), getTimestamp(0), _timestampEndTime, _weiCostOfTokenStage1, _weiCostOfTokenStage2, _weiCostOfTokenStage3, _wallet, _owner, _bountyAdmin, _founderTokenUnlockPhase1, _founderTokenUnlockPhase2, _founderTokenUnlockPhase3, _founderTokenUnlockPhase4, {
            from: accounts[1]
        }).then(function(instance) {
            // Purchase from accounts[1] for 1.2 ether in stage 2 with calling default function
    ...
    it('ICO sale is stopped after the current time has passed the end timestamp', function(done) {
        ...
        Crowdsale.new(getTimestamp(0), getTimestamp(0), getTimestamp(0), getTimestamp(0), _weiCostOfTokenStage1, _weiCostOfTokenStage2, _weiCostOfTokenStage3, _wallet, _owner, _bountyAdmin, _founderTokenUnlockPhase1, _founderTokenUnlockPhase2, _founderTokenUnlockPhase3, _founderTokenUnlockPhase4, {
            from: accounts[1]
        }).then(function(_instance) {
    ...
    it('ICO stage is automatically changed when cap of current stage is exceeded, cannot sale over MAX_ALLOWED_TOTAL cap, and set the correct amount of tokens that founders could withdraw', function(done) {
        ...
        Crowdsale.new(getTimestamp(0), _timestampStage2, _timestampStage3, _timestampEndTime, _weiCostOfTokenStage1, _weiCostOfTokenStage2, _weiCostOfTokenStage3, _wallet, _owner, _bountyAdmin, _founderTokenUnlockPhase1, _founderTokenUnlockPhase2, _founderTokenUnlockPhase3, _founderTokenUnlockPhase4, {
            ...
            const STAGE_3_PURCHASE = web3.toBigNumber(0.0575);
            /**
             ...
             * Max cap of stage 2 is 22% of MAX_SUPPLY
             ...
             * However, since the last stage is over-bought for 24% - 22% = 2%
             * Actual cap for stage 3 should be only 25% - 2% = 23% of MAX_SUPPLY
             * 
             * Here we make 4 purchases, each with 5.75% of MAX_SUPPLY, which should just met the cap
             * All 4 transactions should go through with rate of stage 3 ICO
             * One more purcahses with 5.75% + 1 token will be made before the final purchase to test if it is possible to buy over the set MAX_ALLOWED_TOTAL cap
             * 
            ...
            .then(function() {
                ...
                assert(founderWithdrawablePhase1.cmp(maxSupply.mul(0.13)) == 0, 'Amount of token withdrawable by founders in phase 1 is not identical to 13% of all available token');
    ...
    it('Correct amount of token are set to be withdrawable by founders and rest are burnt when maximum cap is not reached', function(done) {
        ...
        // Total token distributed = 9% (Bought) + 3% (Bounty) = 12%
        // Founder should be able to withdraw 12%/3 = 4% of token, since they will own 25% of the 16% supply available
        var withdrawableTotal = tokenBought.add(maxSupply.mul(web3.toBigNumber(0.03))).div(3);
        var withdrawablePhase1 = withdrawableTotal.mul(0.52); // 13% - 0.25 * 0.52
        var withdrawablePhase2 = withdrawableTotal.mul(0.16); // 4% - 0.25* 0.16
        var withdrawablePhase3 = withdrawableTotal.mul(0.16); // 4% - 0.25* 0.16
        var withdrawablePhase4 = withdrawableTotal.mul(0.16); // 4% - 0.25* 0.16
        // Maximum supply left should be 16% of maxSupply [9% (Bought) + 3% (Bounty) + 4% (Locked)]
        ...
        Crowdsale.new(getTimestamp(0), getTimestamp(0), getTimestamp(0), getTimestamp(0.2), _weiCostOfTokenStage1, _weiCostOfTokenStage2, _weiCostOfTokenStage3, _wallet, _owner, _bountyAdmin, _founderTokenUnlockPhase1, _founderTokenUnlockPhase2, _founderTokenUnlockPhase3, _founderTokenUnlockPhase4, {
            from: accounts[1]
        }).then(function(_instance) {
            instance = _instance;
            ...
            assert(founderWithdrawable[0].cmp(withdrawablePhase1) == 0, 'Token withdrawable to founders in phase 1 does not equate to 13% of total available supply when maximum cap is not reached');
    ...
    it('founderWithdraw allow founders to withdraw correct amount of token after lockup of that phase, and cannot be called after all phase are withdrawn', function(done) {
        ...
        // Buy 9% of token in the whole ICO
        var tokenBought = maxSupply.mul(web3.toBigNumber(0.09));
        // Founder should be able to withdraw [9% (Sale) + 3% [Bounty])/3 = 4% of token, since they will own 25% of the 16% supply available
        var withdrawable = tokenBought.add(maxSupply.mul(web3.toBigNumber(0.03))).div(3);
        // In phase 1, 52% of withdrawable (25% * 0.52 = 13%) can be withdrawn
        var withdrawablePhase1 = withdrawable.mul(web3.toBigNumber('0.52'));
        // In phase 2, 16% of withdrawable (25% * 0.16 = 4%) can be withdrawn
        var withdrawablePhase2 = withdrawable.mul(web3.toBigNumber('0.16'));
        // In phase 3, 16% of withdrawable (25% * 0.16 = 4%) can be withdrawn
        var withdrawablePhase3 = withdrawable.mul(web3.toBigNumber('0.16'));
        // In phase 4, 16% of withdrawable (25% * 0.16 = 4%) can be withdrawn
        var withdrawablePhase4 = withdrawable.mul(web3.toBigNumber('0.16'));
        // Deploy the contract, immediately start stage 3 ICO
        ...
        Crowdsale.new(getTimestamp(0), getTimestamp(0), getTimestamp(0), getTimestamp(0.1), _weiCostOfTokenStage1, _weiCostOfTokenStage2, _weiCostOfTokenStage3, _wallet, _owner, _bountyAdmin, getTimestamp(0.2), getTimestamp(0.2), getTimestamp(0.2), getTimestamp(0.2), {
            from: accounts[1]
        }).then(function(_instance) {
            instance = _instance;
            return purchaseConfirmSale(instance, accounts[1], _wallet, calEther(tokenBought, _weiCostOfTokenStage3), _weiCostOfTokenStage3);
    ...
    it('Cannot founderWithdraw a phase before the unlock time', function(done) {
        ...
        Crowdsale.new(now, now, now, icoEnd, _weiCostOfTokenStage1, _weiCostOfTokenStage2, _weiCostOfTokenStage3, _wallet, _owner, _bountyAdmin, phase1, phase2, phase3, phase4, {
            from: accounts[1]
        }).then(function(_instance) {
            instance = _instance;
            return purchaseConfirmSale(instance, accounts[1], _wallet, calEther(tokenBought, _weiCostOfTokenStage3), _weiCostOfTokenStage3);
    ...
    it('founderWithdraw can only be called by contract owner', function(done) {
        ...
        Crowdsale.new(getTimestamp(0), getTimestamp(0), getTimestamp(0), getTimestamp(0), _weiCostOfTokenStage1, _weiCostOfTokenStage2, _weiCostOfTokenStage3, _wallet, _owner, _bountyAdmin, getTimestamp(0), getTimestamp(0), getTimestamp(0), getTimestamp(0), {
            from: accounts[1]
        }).then(function(_instance) {
            instance = _instance;
            return instance.postICO();
